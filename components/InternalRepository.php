<?php namespace Pensoft\Icp\Components;
...
class InternalRepository extends ComponentBase
{
    ...
    public function onRun(): void
    {
        ...
        $this->page['queries'] = http_build_query(request()->all());
        $this->page['is_download'] = request()->query('download', false);

        if($query = request()->post('query')){
        ...
        $this->page['folders'] = Subfolders::where('pensoft_icp_subfolders.parent_id', null)->get();
        if($this->param('slug')){
            $this->page['folder'] = Subfolders::select('pensoft_icp_subfolders.*')
...
//		}
        ...
        $this->page['function'] = new class {
            ...
            public function get($id) {
                $folder = Subfolders::select('pensoft_icp_subfolders.*')
                ->where('id', $id)
                ->first();
